...
type Config struct {
	...
	// e.g. staging for staging-first workflows. Override per-PR with the
	...
	// Tasks configures the create-time task picker (GitHub issues / ClickUp).
...
type AgentConfig struct {
	...
	// Model is the default model for new sessions. For claude it becomes
	// `--model <model>` (accepts aliases like sonnet/opus/haiku or a full id);
...
type ClickUp struct {
	...
	// Token authorizes the ClickUp REST API for the task picker. Falls back to
	// $CLICKUP_TOKEN when empty.
	Token string `yaml:"token,omitempty" json:"token,omitempty"`
	// Team/Space/List scope the task picker (set by `norn auth`). Empty List/Space
	// means "your assigned tasks" across the workspace.
	Team  string `yaml:"team,omitempty" json:"team,omitempty"`
	Space string `yaml:"space,omitempty" json:"space,omitempty"`
	List  string `yaml:"list,omitempty" json:"list,omitempty"`
}

// TasksConfig selects the create-time task picker source.
type TasksConfig struct {
	Provider string `yaml:"provider,omitempty" json:"provider,omitempty"` // github | clickup | none
}
