...
func TestEditAddsAbsentKeys(t *testing.T) {
	...
	e.SetString([]string{"pr_base"}, "user_test")
	...
	if v, ok := e2.GetString([]string{"pr_base"}); !ok || v != "user_test" {
...
func TestEditSeq(t *testing.T) {
	p := writeTemp(t, "worktree_dir: ~/worktrees\n")
	e, _ := OpenEditor(p)
	e.SetStringSeq([]string{"base_branches"}, []string{"master", "user_test"})
	if err := e.Save(); err != nil {
		t.Fatal(err)
	}
	out := readBack(t, p)
	if !strings.Contains(out, "base_branches:") || !strings.Contains(out, "master") || !strings.Contains(out, "user_test") {
...
func TestEditDelete(t *testing.T) {
	p := writeTemp(t, "pr_base: user_test\nai_naming: true\n")
	e, _ := OpenEditor(p)
