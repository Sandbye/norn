...
`l` or `→` opens the role's conversation: one line per thing the agent said or ran, following the log while it works, with a prompt at the bottom. A headless role has no session to open, so this is where you watch it and where you talk to it. What you type resumes that role's finished session, and both halves of the exchange are appended to the same log, so the pane holds the whole conversation. `tab` cycles what a reply may do, `esc` goes back. norn never handles a credential for any of this: it runs the agent's own binary, which authenticates itself, and reads the output that binary writes.
